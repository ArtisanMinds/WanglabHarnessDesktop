...
[预期结果] 1. 禁用后 bundles 中不再包含该插件名。2. 重启后该插件仍为禁用态。3. 其宿主路由返回 404（与 `01-dsh-host-and-core-contract.md` TC-CORE-L2-01-006 的判定一致）。
[清理] `enable_dsh_plugin` 恢复启用；`DELETE /session/<id>`

### [P3] [反向] 验证插件运行期报错被上报且壳层给出恢复入口

[Case ID] TC-XP-L3-16-003
[层级] L3（真实 Tauri 窗口）
[类型] 异常
...
| --- | --- | --- | --- |
| 内置插件清单与自愈 | TC-XP-L3-16-001 | 正向 | 依赖 10 个插件全部构建 |
| 禁用/启用链路 | TC-XP-L3-16-002 | 正向 | 会改用户 profile，需在隔离数据目录执行 |
| `dsh://plugin-error` 上报 | TC-XP-L3-16-003 | 异常 | 需要测试替身插件 |
| invoke 白名单 | TC-XP-L3-16-004 | 边界 | 需要驱动注入脚本 |
| 安全模式 | TC-XP-L3-16-005 | 异常 | 会改用户状态，需隔离数据目录 |
| boot 卡死探测 | TC-XP-L3-16-006 | 边界 | 需要网络阻断能力 |
| iframe 地址不含 token | TC-XP-L3-16-007 | 回归 | — |
| 路由互不遮蔽 | TC-XP-L2-16-001 | 边界 | 可并入 `02` 的文件执行 |
...
- **G-XP-2**：TC-XP-L3-16-003 与 TC-XP-L3-16-006 需要**测试替身插件**（必然抛错 / 让 bundle 挂起）。仓库当前无此类 fixture，属新增基础设施，需单独批次授权。
...
- **假设**：iframe 与宿主**跨源**（宿主 `tauri://localhost`，iframe `http://127.0.0.1:<port>`）。WebDriver 切 frame 需要 WebView2 原生帧执行（`ICoreWebView2Frame2::ExecuteScript`），已由 vendored patch 打通（见 `desktop/02-config-locale.md` §6 与 `src-tauri/vendor/tauri-plugin-wdio-webdriver/PATCH.md`）；上游的 `contentWindow.eval` 模拟在跨域下不可用。若 CSP 收紧，`use-iframe-message.ts:25` 的 origin 校验会成为断言前置。
