...
> 层级：L3（真实 Tauri 窗口）
> 自动化：`test/e2e/desktop/08-backup-restore.e2e.ts`（待建立）
> 前置：见 `00-overview.md` §5.1；`dist/` 与 debug 二进制已按最新源码重建
> 运行：`vitest --project desktop -- test/e2e/desktop/08-backup-restore.e2e.ts`

备份子视图的创建、还原、还原为新档案与删除。
...
[自动化] 待接线（`test/e2e/desktop/08-backup-restore.e2e.ts`）
[前置条件] 「档案」面板已渲染
...
| 实现位置 | 覆盖 Case ID | 类型 |
| --- | --- | --- |
| ``src/ui/config/profile.tsx:241-245`、`:285-294`` | `TC-DSK-L3-08-001` | 正向 |
| ``src/ui/config/backup.tsx:60-69`` | `TC-DSK-L3-08-002` | 正向 |
| ``src/ui/config/backup.tsx:71-119`` | `TC-DSK-L3-08-003` | 异常 |

---

### 缺口与假设

- **G-D08-1**：还原会**改写 profile 目录**，是本套中破坏性最强的操作。接线时必须在独立数据目录下执行，且清理步骤要能恢复原 profile 内容（或直接重建环境）。
- **G-D08-2**：备份产物（时间戳、大小、是否含凭据）的内容级校验需要解包能力（归档格式未在本次调查中确认）。当前只断言「列表出现新项且大小大于 0」。
- **G-D08-3**：`waitForHarnessStopped` 的错误分类逻辑（`502|ECONNREFUSED|ETIMEDOUT` 视为 transient 继续重试，其余视为已停止）**未覆盖**；该分支属纯逻辑，应在单元测试覆盖，而非 L3。
- **假设**：还原后会自动异步 `launch_harness`（`backup.tsx:100-102`），因此 TC-DSK-L3-08-003 的清理可直接等待服务恢复，无需手动点击重启。
