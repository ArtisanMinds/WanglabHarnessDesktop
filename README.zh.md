...
- **零环境** — 首次启动自动装配内置 Node 运行时与 Harness 内核；本机已有兼容 Node（v22.15+ / v23.8+）时直接复用，不碰你的系统环境。
...
- **命令行集成** — 安装后自动注册 `dsh` / `pnpm` 命令（`%LOCALAPPDATA%\deepseek-harness\bin` / `~/.local/bin`），新开终端即用。
