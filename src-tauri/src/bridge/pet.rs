...
fn sprite_grid(declared: Option<u8>, width: u32, height: u32) -> Result<SpriteGrid, String> {
    ...
    let rows = if version == PET_SPRITE_V1 {
        PET_SPRITE_V1_ROWS
    } else {
        PET_SPRITE_V2_ROWS
    };
...
mod tests {
    ...
    #[test]
    fn sprite_grid_preserves_market_frame_dimensions_and_resolves_ambiguous_layouts() {
        for (declared, height, version, rows) in [
            (None, 1872, 1, 9),
            (Some(1), 1872, 1, 9),
            (Some(2), 1872, 1, 9),
            (None, 2288, 2, 11),
            ...
            let grid = sprite_grid(declared, 1664, height).unwrap();
            assert_eq!((grid.version, grid.rows), (version, rows));
            assert_eq!((grid.frame_width, grid.frame_height), (208, 208));
        }
        assert_eq!(sprite_grid(None, 800, 990).unwrap().rows, 11);
        assert_eq!(sprite_grid(Some(1), 800, 990).unwrap().rows, 9);
        assert_eq!(sprite_grid(Some(2), 800, 990).unwrap().rows, 11);
    }
