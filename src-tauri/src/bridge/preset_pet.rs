...
mod tests {
    ...
    #[test]
    fn shipped_catalog_has_no_default_pets() {
        ...
        assert!(catalog.is_empty(), "默认宠物必须由用户从市场选择");
    }

    #[test]
    fn remote_catalog_supports_macos_media_override() {
        let spec: PresetPetSpec = serde_json::from_value(serde_json::json!({
            "id": "remote-fixture",
            "name": "Remote fixture",
            "kind": "dsh",
            "config": "https://example.com/config.jsonc",
            "uri": {
                "default": "https://example.com/webm",
                "mac": "https://example.com/mov"
            },
            "ext": { "default": "webm", "mac": "mov" }
        }))
        .unwrap();
        assert_eq!(spec.uri.default, "https://example.com/webm");
        assert_eq!(spec.uri.mac.as_deref(), Some("https://example.com/mov"));
        assert_eq!(
