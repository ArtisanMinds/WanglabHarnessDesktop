//! 桌面应用自更新模块。
//!
//! 与 `dsh` 内核更新（`download` 模块）不同，这里负责 Wanglab Harness Desktop
//! 自身的更新：读取 WanglabAI 发布清单、下载安装包、并交给系统打开安装器。
//!
//! 设计考量：
//! - 每次检查都实时读取 WanglabAI `latest.json`，不依赖 GitHub。
...
const UPDATE_MANIFEST_URL: &str =
    "https://seuwanglab.com/downloads/wanglab-harness-desktop/latest.json";
/// 版权信息（与 tauri.conf.json bundle.copyright 保持一致）
const COPYRIGHT: &str = "Copyright © 2026 WanglabAI";
/// About 对话框的 "Powered by" 文案
const POWERED_BY: &str = "Wanglab Harness Desktop";
/// AppData 下安装包存放目录名
