...
/// 读取 Wanglab 自有 Core 发布清单。
///
/// `assets` 兼容当前站点使用的对象形式（文件名作为键），也兼容桌面更新
/// 清单使用的数组形式。这样 Desktop 下载的文件、tag 和摘要始终来自同一份
/// 本地发布元数据，不会把自有 ZIP 与上游 GitHub 摘要错配。
async fn fetch_hosted_dsh_pkg_info() -> Result<LatestDshPkg, String> {
    let client = github_client()?;
    let body = client
        .get(config::DSH_CORE_METADATA_URL)
        .send()
        .await
        .map_err(|e| format!("Wanglab Core metadata request failed: {e}"))?
        .error_for_status()
        .map_err(|e| format!("Wanglab Core metadata request failed: {e}"))?
        .text()
        .await
    ...
    let asset_value = release
        ...
        .ok_or_else(|| format!("Wanglab Core metadata has no {expected_name} asset"))?;
    let asset: HostedDshAsset = serde_json::from_value(asset_value.clone())
        .map_err(|e| format!("Invalid Wanglab Core asset metadata: {e}"))?;
    let asset_url = if asset.url.trim().is_empty() {
        config::get_dsh_download_url_for_tag(&release.tag)?
    } else {
        asset.url
    };
    let commit = release
        ...
        .unwrap_or_else(|| commit_fallback_from_tag(&release.tag));
    ...
    Ok(LatestDshPkg {
        ...
        commit,
        asset_url,
        digest: asset.digest.filter(|value| value.starts_with("sha256:")),
    })
}
...
pub async fn fetch_latest_dsh_pkg_info() -> Result<LatestDshPkg, String> {
    if config::DSH_CORE_URL.starts_with("https://seuwanglab.com/") {
        match fetch_hosted_dsh_pkg_info().await {
            Ok(info) => return Ok(info),
            Err(error) => log::warn!("Wanglab Core metadata unavailable, using upstream: {error}"),
        }
    }
    let client = github_client()?;
    let expected_name = config::get_dsh_download_url()?
...
pub async fn fetch_dsh_pkg_version(version: &str) -> Result<LatestDshPkg, String> {
    if config::DSH_CORE_URL.starts_with("https://seuwanglab.com/") {
        if let Ok(info) = fetch_hosted_dsh_pkg_info().await {
            if parse_version_from_tag(&info.tag).as_deref() == Some(version) {
                return Ok(info);
            }
        }
    }
