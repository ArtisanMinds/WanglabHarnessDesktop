...
//! 幂等与容错：
//! - 已含标记即跳过；
//! - 上游将来自己放宽该判定（`chipTitle === void 0 && cwd === void 0`）时只补能力标记，
//!   不再改写源码——补丁随上游自动退休；
...
/// 官方 inert 判定（锚点绑定 0.1.5-rc.1 / rc.2 的压缩后源码）。
const INERT_ANCHOR: &str = "const inert = sessionId === void 0 || hero && chipTitle === void 0;";

/// 放宽后的 inert 判定：会话有 cwd 就不再算 inert。
const INERT_PATCHED: &str =
    "const inert = sessionId === void 0 || hero && chipTitle === void 0 && cwd === void 0;";

/// 上游自己放宽后的形态（只用于判定「无需再改源码，只补能力标记」）。
const INERT_UPSTREAM_FIXED: &str = "chipTitle === void 0 && cwd === void 0";
...
fn patch_source(source: &str) -> PatchOutcome {
    ...
    let needs_relax = !source.contains(INERT_UPSTREAM_FIXED);
    if needs_relax && !source.contains(INERT_ANCHOR) {
    ...
    if needs_relax {
        patched = patched.replacen(INERT_ANCHOR, INERT_PATCHED, 1);
    }
...
mod tests {
    ...
    fn relaxes_inert_and_declares_capability() {
        ...
        assert!(!patched.contains(INERT_ANCHOR));
    ...
    fn upstream_fix_only_declares_capability() {
        ...
        assert!(patched.contains(INERT_UPSTREAM_FIXED));
    ...
    fn skips_when_anchor_missing() {
        ...
        let no_return = format!("\t\t\t{INERT_ANCHOR}\n");
