...
| Case ID | 用例 | 裁剪原因 |
| --- | --- | --- |
| `TC-DSK-L3-13-006` | 验证 CLI link 开关创建与删除 shim | 纯逻辑断言，下沉单元测试层 |

---
| `TC-DSK-L3-13-003` | 验证缩放选择的持久化（写读设置项） | 纯逻辑断言，下沉单元测试层 |
| `TC-DSK-L3-13-005` | 验证关闭行为选择的持久化（写读设置项） | 纯逻辑断言，下沉单元测试层 |

## 6. 追踪矩阵

| 实现位置 | 覆盖 Case ID | 类型 |
| --- | --- | --- |
| ``src/ui/config/debug.tsx:141-171`；issue #91` | `TC-DSK-L3-13-001` | 正向 |
| ``src/ui/config/debug.tsx:143-146`、`:162-170`` | `TC-DSK-L3-13-002` | 异常 |
| ``src/ui/config/components/launch-on-login.tsx:10-23`` | `TC-DSK-L3-13-004` | 正向 |
