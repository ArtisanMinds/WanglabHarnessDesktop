...
合计 **28 个用例文件、105 条 L3 用例**（`00` 不承载用例本体；原 `17` 桌宠批次已移出）。

> **范围收敛（2026-09）**：原 275 条已按「P1 全保留 + 每批次补足 3 条（安全边界优先）」收敛为 **105 条** E2E 用例。被裁条目分两类：① 纯后端逻辑（函数/时序/协议）**下沉单元测试层**，共 **107** 条，在各文件「单元测试层」小节保留记录；② 无用户可见后果或重复断言面的 UI 细节**直接删除**。`17` 桌宠批次整章移出至插件用例集。已接线的 `01`–`04`（26 条）不受影响。
...
| `18-notification-download.md` | `18-001`、`18-003`、`18-004` | 3 | 2 / 1 / 0 / 0 |
...
合计 **105** 条 L3 用例：正向 66 / 异常 27 / 边界 11 / 低频 1；另有 **107** 条纯后端逻辑条目标记为「单元测试层」（见各文件对应小节），不计入 E2E。已接线 26 条（`01`–`04`）。
