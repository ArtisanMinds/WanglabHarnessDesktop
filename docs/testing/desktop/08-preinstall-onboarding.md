...
[自动化] 待接线（同上）
[前置条件] TC-DSK-L3-08-002 通过；默认勾选集合大小 ≥ 2
...
[Case ID] TC-DSK-L3-08-005
[层级] L3（真实 Tauri 窗口）
[类型] 正向
[追踪] `src/layout/components/setup-preinstall.tsx:70-79`、`:260-261`
[自动化] 待接线（同上）
[前置条件] TC-DSK-L3-08-001 通过；存在已安装的候选插件
...
[Case ID] TC-DSK-L3-08-009
[层级] L3（真实 Tauri 窗口）
[类型] 边界
[追踪] `src/layout/components/setup-preinstall.tsx:186-190`
[自动化] 待接线（同上）
[前置条件] TC-DSK-L3-08-001 通过；执行环境允许拉起系统浏览器
...
- **G-D08-2**：TC-DSK-L3-08-006/059 会真实执行 `dsh plugin` 安装，**改动用户档案的插件集合**。按 `00-overview.md` G8，测试必须使用独立数据目录并自行清理，否则会污染 `09` 的前置状态。
