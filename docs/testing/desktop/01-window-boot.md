...
[自动化] 是（`test/e2e/desktop/01-window-boot.e2e.ts`）
[前置条件] TC-DSK-L3-001 通过；无历史窗口几何持久化记录
[测试数据] 期望宽 `1280`、高 `840`
[测试步骤] 1. 清空窗口几何持久化记录。2. 拉起应用。3. 读取窗口内尺寸与 `screen` 尺寸（均按 `devicePixelRatio` 还原）。
[预期结果] 1. 清空成功。2. 会话建立成功。3. 实际尺寸同时满足：不超过申请值 `1280×840`（应用未自行放大）、不超过屏幕（不得越出显示器）、且不低于 `min(申请值, 屏幕)`（屏幕放得下就必须给足）。三项均允许 ±2px 取整误差。
[清理] 恢复被清空的几何记录；`DELETE /session/<id>`

> **为什么不直接断言等于 `1280×840`**：屏幕放不下时窗口会被系统夹进屏幕。CI 的虚拟显示器小于该尺寸（实测宽被夹到 `1024`），等比断言会在 CI 上必然失败。上界 + 屏幕 + 下界的三段式断言在两种环境下都成立，且仍能捕获「窗口开小了」与「窗口开大了」两类回归。
