...
| 01 | 编排骨架与共享路由契约 | `01-dsh-host-and-core-contract.md` | 已验证 | `test/e2e/plugins/01-dsh-host-and-core-contract.e2e.ts` 12 例全绿（编排骨架 6 + 共享路由契约 6）；编排新增导出 `assertMountRegistered`；跨源 403 期望值按实测修正（上游围栏 `forbidden`，路由层 `cross-origin-request` 在 L2 不可达，由 L1 覆盖） |
