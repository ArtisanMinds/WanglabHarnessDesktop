...
| 02 | `dsh-tauri-pet`（SSE → 客户端 → 桌面端窗口） | `02-dsh-tauri-pet.md` | 13 | 13 | 已验证 | L2 段 5 例 + 浏览器段 4 例在 `test/e2e/plugins/02-dsh-tauri-pet.e2e.ts`（9 例），4 条 Tauri 原生窗口用例在 `test/e2e/desktop/02-pet-window.e2e.ts`（跨文件例外）；实测第 2 帧心跳 15.01–15.02s、重连首个响应块即就绪。G-PET-9（侧栏入口 30s 内未翻转 `aria-pressed`）、G-PET-10（`set_pet_size` 越界返回 `PET_SIZE_OUT_OF_RANGE` 而非夹紧）记录在案。 |
...
| 05 | `dsh-tauri-worktree` | `05-dsh-tauri-worktree.md` | 16 | 11 | 已验证 | e2e 7 例全绿（bindings 形状、未绑定回落 `local`、两处缺参 400、删除幂等失败体 200、checkouts 未绑定 400）；`-006` 实测失败体只有 `{error}` 无 `ok`，文档预期按实测收紧；G-WT-4 固化为疑似缺陷、G-WT-5 记录「未创建目录」以回读 `GET /bindings` 为空替代；另新增 §3.1 四条 L1 用例（`TC-WT-U-05-001`–`004`）覆盖 `#648` 的「`isGit` 三态可见性 + 发送拦截同条件」回归（`packages/dsh-tauri-worktree/src/client/components/mode-select.utils.test.ts`，`unit` project 17 例全绿）；`TC-WT-C-05-004`（创建失败回落 `local`）待接线 |
...
**合计**：`01`–`11` 共 **11 个用例文件**，`[Case ID]` **136** 条（按 `^[Case ID]` 字段行计）、`[自动化] 是` **100** 条 = e2e **96**（`plugin` 车道 92 + `desktop` 车道 4）+ 批次 `05` 的 **4** 条 L1 单元层；L2/L3 层与代码 `it()` **逐文件相等**，其余为手工执行 / 待接线条目。逐文件明细见 [`plugins/00-overview.md`](./plugins/00-overview.md) §7.1 追踪矩阵。
