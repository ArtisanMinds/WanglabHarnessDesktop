...
| --- | --- | --- | --- |
| `05-profile/05` 验证不同档案安装的插件互不影响 | TC-ISO-L3-15-001 | 正向 | — |
| `05-profile/05` 验证不同档案的补丁设定各自独立 | TC-ISO-L3-15-002 | 正向 | — |
| `05-profile/05` 验证切换档案后服务以新档案启动且旧档案数据完整保留 | TC-ISO-L3-15-003 | 正向 | 档案切换本身由 `../desktop/03-profile.md` 覆盖 |
| `05-profile/05` 验证删除档案 A 不影响其他档案 | TC-ISO-L3-15-004 | 边界 | — |
| `06-plugin/03` 验证 pnpm-workspace.yaml 多文档被归一化并恢复插件安装 | TC-ISO-L3-15-005、TC-ISO-L3-15-006 | 正向 / 异常 | — |
| `01-install/05` 验证数据目录不可写时在写入前给出可执行权限诊断 | TC-ISO-L3-15-007 | 异常 | 系统权限无法在页面内构造，标为手工 |
| 现行实现新增（禁用清单的档案局部性） | TC-ISO-L3-15-008 | 正向 | 归档套件未覆盖 |

---

## 4. 缺口与假设

- **G-ISO-1**：TC-ISO-L3-15-007 需要修改文件系统权限，无法由页面内测试自足完成，按 `desktop.test.md` 的规定标为 `[自动化] 否（手工）`，并在执行记录中附上目录属主信息。
