...
**与桌面端套件的分工**：`../desktop/06-plugin-lifecycle.md` 断言引导页的渲染、勾选交互与按钮；本文件断言**预设清单的来源与字段语义、指纹判定、安装命令的实际形态、取消与跳过的持久化结果**。共同覆盖归档套件 `06-plugin/04` 与 `01-install/04`。
...
| --- | --- | --- | --- |
| `06-plugin/04` 验证选中推荐插件后确认安装并展示实时安装日志 | TC-PRE-L3-13-003 | 正向 | 日志逐行渲染由 `../desktop/06` 断言 |
| `06-plugin/04` 验证安装按当前档案执行 dsh plugin --profile add 命令 | TC-PRE-L3-13-003、TC-PRE-L3-13-004 | 正向 / 异常 | — |
| `06-plugin/04` 验证跳过引导后记录完成不再弹出 | TC-PRE-L3-13-005 | 正向 | — |
| `06-plugin/04` 验证取消进行中的预装插件安装 | TC-PRE-L3-13-007 | 异常 | — |
| `06-plugin/04` 验证 Windows 下列出 dsh-win-terminal-inspector 修复项 | TC-PRE-L3-13-002（反向）+ `../desktop/06` | 边界 | Windows 上需正向断言，见 G-PRE-2 |
| `06-plugin/04` 验证打开预装插件仓库地址 | TC-PRE-L3-13-008 | 边界 | — |
| `06-plugin/04` 验证预检通过但 pnpm shim 找不到 node 时安装仍成功（issue #121） | TC-PRE-L3-13-009 | 边界 | — |
| `01-install/04` 验证预设清单指纹变更后重新进入引导 | TC-PRE-L3-13-006 | 边界 | — |
| `01-install/04` 验证首次启动列出预设插件清单且推荐/修复/默认项默认勾选 | TC-PRE-L3-13-001 + `../desktop/06` | 正向 | 勾选交互归 desktop 06 |
...
- **G-PRE-2**：TC-PRE-L3-13-002 在 Windows 工作站上无法执行（需非 Windows）。若必须覆盖 Windows 正向分支，需新增一条「Windows 下列出 `winOnly` 修复项」用例，但当前 `preset-plugins.json` 内已无 `winOnly` 条目（字段语义仍保留），故该分支**暂时无数据可测**。
