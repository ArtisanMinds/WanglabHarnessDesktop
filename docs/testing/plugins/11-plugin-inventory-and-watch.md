...
**与桌面端套件的分工**：`../desktop/06-plugin-lifecycle.md` 断言「界面上看到什么」；本文件断言**清单真值从哪来、文件改了会怎样、坏数据如何降级**——即落盘与事件层。两者共同覆盖归档套件 `06-plugin/01-已安装插件列表与监控` 的 5 条用例。
...
[Case ID] TC-INV-L3-11-005
[层级] L3（真实 Tauri 窗口）
[类型] 边界
[追踪] `src-tauri/src/service/plugin/watch.rs:30`
[自动化] 待接线（同上）
[前置条件] 同 TC-INV-L3-11-004
...
| --- | --- | --- | --- |
| `06-plugin/01` 验证已安装插件列表只读且正确展示 | TC-INV-L3-11-001 | 正向 | 只读性（无编辑控件）由 `../desktop/06-plugin-lifecycle.md` 断言 |
| `06-plugin/01` 验证列表展示插件名与版本等关键信息 | TC-INV-L3-11-002、TC-INV-L3-11-003 | 正向 / 边界 | — |
| `06-plugin/01` 验证插件文件变化后轮询检测并推送 dsh-plugins-updated 事件 | TC-INV-L3-11-004、TC-INV-L3-11-005 | 正向 / 边界 | — |
| `06-plugin/01` 验证插件被移除后列表能反应 | TC-LIFE-L3-12-007（见 `12-plugin-lifecycle-commands.md`） | 正向 | 卸载副作用归 12 |
| `06-plugin/01` 验证插件自身 package.json 缺失或损坏时列表仍展示该插件 | TC-INV-L3-11-003 | 边界 | — |
| 现行实现新增（禁用 vs 未加载、包名别名 patch 禁用） | TC-INV-L3-11-007、TC-INV-L3-11-008 | 正向 / 边界 | 归档套件无对应用例 |
| 无 manifest 降级 | TC-INV-L3-11-006 | 异常 | 归档套件无对应用例 |

---

## 4. 缺口与假设

- **G-INV-1**：清单真值的稳定断言依赖「`dependencies` 里恰好只有插件」这一前提。若用户档案里混入非插件依赖，TC-INV-L3-11-001 的差异比较需加入白名单，届时以 `internal-plugins.json` / `preset-plugins.json` 的 id 集合为基准。
- **G-INV-2**：事件监听能力在页面内不可直接订阅 Tauri 事件（iframe 只能 `dsh://tauri:invoke` 白名单内的 9 条命令，见 `00-overview.md` §5.2）。TC-INV-L3-11-004/005 需由壳层测试侧记录事件计数后再经 DOM 或测试通道回读；接线方式待定。
- **G-INV-3**：`get_dsh_plugins` 声明为非 `Result`，因此「读失败」只能表现为空数组，无法与「真为空」区分（`src-tauri/src/bridge/plugin.rs:127`）。TC-INV-L3-11-006 以「界面显示空态而非错误态」作为区分代理。
