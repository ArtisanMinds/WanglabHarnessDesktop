...
[自动化] 是（同上文件 → `describe('共享路由契约')`）
[前置条件] 该路径的插件 id 在仓库内不存在（永不挂载）；请求带会话 Cookie
[测试数据] `GET /api/desktop/dsh-tauri-unmounted-probe/ping`
[测试步骤] 1. 发起请求。2. 读状态码与响应体。
[预期结果] 1. 状态码 404（既不是 401 也不是 200）。2. 响应体不含任何插件领域错误（如 `会话不存在或尚未就绪`），与 09 批次同状态的领域 404 形成对照——证明失败来自路由缺失，而非鉴权围栏或会话缺失。
[清理] 无
