...
test/e2e/
  ...
  support/
    dsh-host.ts                    # 宿主环境脚手架：DSH_HOME 隔离、Profile 构造、挂载、启动、鉴权与回收
...
* **文件拆分**：允许同一插件拆分不同层级（如桌宠插件：L2/C 位于 `plugins/dsh-tauri-pet.e2e.ts`，L3 窗口测试位于 `desktop/02-pet-window.e2e.ts`，以避免 Linux CI 环境跳过 Windows-only 的 Tauri 测试）。
...
由 `test/e2e/support/dsh-host.ts` 提供底座，`test/e2e/global-setup.ts` 实例化全局单一共享宿主，通过 `project.provide()`（`inject()`）跨 Worker 下发参数。
