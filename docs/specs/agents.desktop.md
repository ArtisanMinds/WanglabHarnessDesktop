...
* **内置插件 spec 预加引号随核心版本 (Issue #647)**：`dsh plugin` 在 `< 0.1.6-alpha.2`（含 0.1.5-rc.2 / 0.1.6-alpha.1）用 `spawnSync("pnpm", args, { shell: process.platform === "win32" })` 把参数拼成命令行，Node 对 `shell:true` 只按空格拼接、不转义（DEP0190），含空格的 `link:<应用安装目录>` 必须预加双引号才不会被切碎；`0.1.6-alpha.2` 起改经 `@deepseek-ai/dsh-plugin-manager` 用 execa 以 **argv 数组**启动 pnpm 并自行按 cmd 规则转义（`arguments/command-file.js`），预加引号会原样进入 spec → pnpm 报 `ERR_PNPM_SPEC_NOT_SUPPORTED_BY_ANY_RESOLVER`，启动卡在 `INTERNAL_PLUGIN_INSTALL_FAILED`。`install::spec::spec_argument` 按 `core::active_version` 门控引号化：老核心 + Windows 才加，版本读不到 / 解析失败按新核心处理（预加引号对新核心是必然失败，不预加只在「老核心 + 含空格路径」下失败），macOS/Linux 恒不加引号（issue #104）。
