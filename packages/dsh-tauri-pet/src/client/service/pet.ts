...
/** 清单仅来自应用内目录，避免读取其他应用的数据。 */
export async function loadPetCatalog(): Promise<PetActionResult> {
  const revision = store.pet.beginFetch()
  try {
    const [status, chat, presets] = await Promise.all([
      getPetStatus(),
      getPetList('chat'),
      getPresetPets(),
    ])
    store.pet.setCatalog({ presets, chat })
    store.pet.commitFetch(revision, status)
    return { ok: true }
  }
  catch (error) {
...
export async function enablePet(input: { id: string }): Promise<PetActionResult> {
  try {
    await postActivePet(input.id)
    store.pet.setStatus(await postPetEnabled(true))
    return { ok: true }
  }
  catch (error) {
...
export async function importPetArchive(input: { name: string, data: string }): Promise<PetActionResult> {
  try {
    ...
    store.pet.setChatPets(await getPetList('chat'))
    return { ok: true }
  }
  catch (error) {
