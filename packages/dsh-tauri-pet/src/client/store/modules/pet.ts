  ...
  actions: {
    ...
    commitFetch(revision: number, status: PetStatus): boolean {
      if (revision !== this.fetchRevision)
        return false
      if (this.status && status.revision < this.status.revision)
        return false
      this.status = status
      return true
    },
    ...
    setStatus(status: PetStatus | null): void {
      if (status && this.status && status.revision < this.status.revision)
        return
      this.fetchRevision += 1
      this.status = status
    },
    setCatalog(input: { presets: PresetPetItem[], chat: PetListItem[] }): void {
      this.presetPets = input.presets
      this.chatPets = input.chat
      this.catalogLoaded = true
    },
    setChatPets(list: PetListItem[]): void {
      this.chatPets = list
    },
