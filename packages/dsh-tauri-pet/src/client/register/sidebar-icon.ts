...
import { togglePet } from '../service/pet'
...
function iconActive(): boolean {
  return isPetVisible(store.pet.$state.status)
}
...
async function toggleEnabled(): Promise<void> {
  ...
  if (!status?.active_pet)
    return
  await togglePet({ enabled: !(status.enabled && status.visible) })
}
...
function registerSidebarPetIcon(controller: RegisterController): void {
  ...
  function sync(): void {
    ...
    button.disabled = !status?.active_pet
    const label = status?.error ? `${locale.text('loadFailed')}: ${status.error}` : locale.text(status?.active_pet ? 'name' : 'noPetSelected')
    button.setAttribute('data-tip', label)
    button.setAttribute('aria-label', label)
  }
  controller.add(store.pet.$subscribe(sync))
  sync()
