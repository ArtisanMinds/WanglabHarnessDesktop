...
describe('decoratePetMenuItem', () => {
  ...
  it('disables the action when no pet is selected', () => {
    const item = decoratePetMenuItem(fakeItem('设置'), '请先在宠物设置中选择一只宠物', true)
    const clone = item as unknown as FakeNode

    expect(clone.disabled).toBe(true)
    expect(clone.attributes['aria-disabled']).toBe('true')
  })
})
