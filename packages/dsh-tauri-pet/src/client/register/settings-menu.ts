...
export const settingsMenuFeature = defineRegister((controller) => {
  ...
  function patchMenu(settingsItem: HTMLButtonElement): void {
    ...
    if (settingsItem.closest(MENU_ITEM_WRAP_SELECTOR) === null)
      return
    const selected = Boolean(store.pet.$state.status?.active_pet)
    const label = selected
      ? locale.text(enabled() ? 'closePet' : 'enablePet')
      : locale.text('noPetSelected')
    const item = decoratePetMenuItem(settingsItem, label, !selected)
    if (item === null)
      return
  ...
  controller.listen('click', (event) => {
    ...
    if (!(origin instanceof Element))
      return
    const item = origin.closest<HTMLButtonElement>(`[${PET_MENU_ITEM_ATTRIBUTE}]`)
