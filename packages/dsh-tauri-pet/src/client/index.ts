/**
 * client/index.ts — dsh-tauri-pet 客户端插件体（browser half）：桌宠浮控件。
 *
 * 能力（slot-shadow，零结构补丁，零新增运行时依赖）：
 *   注册进 layout 声明的 `shell.overlay`（list/root 槽，第三方通用浮层）——
 *   右下角胶囊开关键 + 展开的「宠物」设置卡片：启用/停用、显示/隐藏、选择
 *   桌宠（Codex/Deepseek），全部经 dsh-tauri invoke 桥调用桌面端 Tauri 命令
...
import { installPetControl } from './register/control'

/** 插件显示名（诊断元数据）。 */
export const name = PET_CLIENT_PLUGIN

/** 需要的客户端服务：slots（注册 shell.overlay 浮层）、locale（双语文案）。 */
export const inject = ['slots', 'locale']

/**
 * 插件体：安装文案、样式，并注册桌宠浮控件到 shell.overlay。
