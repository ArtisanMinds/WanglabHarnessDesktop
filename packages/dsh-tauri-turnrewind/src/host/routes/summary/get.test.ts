...
const SUMMARY_PATH = '/api/desktop/dsh-tauri-turnrewind/summary'
...
function turnAt(overrides: Partial<TurnRecord> = {}): TurnRecord {
  return {
    turn: 1,
    beforeRef: 'refs/turnrewind/s/1/before',
    afterRef: 'refs/turnrewind/s/1/after',
    files: [],
    insertions: 0,
    deletions: 0,
    createdAt: 1,
...
describe('summary 200 载荷', () => {
  ...
  it('hasBaseline 只认非空 refs', async () => {
    vi.mocked(ledger.load).mockResolvedValue(ledgerWith([
      ...
      turnAt({ turn: 2, beforeRef: 'refs/turnrewind/s/2/before', afterRef: '' }),
      turnAt({ turn: 3, beforeRef: '', afterRef: 'refs/turnrewind/s/3/after' }),
    ]))
