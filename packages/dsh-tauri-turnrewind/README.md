...
设计与决策记录：[`docs/plugins/expired/11.优化计划.turnrewind实现.done.md`](../../docs/plugins/expired/11.优化计划.turnrewind实现.done.md)
...
└─ locks/<sha256(worktree 根)[0:24]>.lock         # 工作区互斥锁（宿主进程之间，见「并发」）
```
...
- **忽略规则**：完全委托源仓库（`.gitignore` / global excludes），并镜像源仓库的
  ...
  **被忽略的路径不进 exclude pathspec**：`git add --all -- . :(exclude)<ignored>` 会以
  ...
  不这么做会让**每一轮**捕获都失败。
- **并发（两道闸门，缺一不可）**：捕获、结算、实时读数、容量治理、撤销全部走**同一工作区级队列**。
  - **进程内 FIFO**（`service/queue.ts`）：同一进程里的任务按提交顺序串行；
  - **跨进程文件锁**（`service/lock.ts`）：同一个 `$DSH_HOME` 下可能同时跑着第二个宿主进程
    （桌面端重启交叠、手动再起的 `dsh web`、离线维护脚本），内存队列对它们无效——
    两个进程同动一份私有仓 index/refs，轻则撞 `index.lock` 让整轮快照失败，重则读到半更新的
    index、或让一方刚写的 refs 被另一方的容量治理清掉。
  队列键统一走 `workspaceKey()`（Windows 上 `C:\Repo` 与 `c:\repo` 是同一个工作区），
  因此两条道、两把锁不会因拼写不同而分裂。顺序是**先排队再拿锁**：反过来会让同一条道里的
  下一个任务去跟自己人争文件锁，白烧等待预算。
  锁的形态：`locks/<hash>.lock`，`openSync(path, 'wx')` 原子创建，内容 `{ pid, token, acquiredAt }`；
  释放只删 token 仍是自己的那一把。
  **只有两种锁会被接管，活着的持有者永不被抢**：内容读不出持有者（崩溃残骸）、记录里的 pid
  已不存在（`EPERM` 视为存活：进程在、只是没权限发信号）。接管是「**改名之前复核一次、改名之后
  再复核一次**」——`rename` 只保证路径级只有一个赢家（避免「两个进程都删都建、于是都以为拿到了
  锁」），**它不是带内容条件的原子 CAS**（Node 没有 `flock`）：两次复核之间发现内容变了，就把
  抢到的那份**放回锁位**并让出。反方向由获取后的**回读自检**兜住（防「创建完成 → 写入完成」
  之间被当成残骸抢走而双持）；两者缺一不可。
  残余窗口（如实记录）：从「复核」到「真去改名」之间若第三方恰好把锁位换成了**活锁**，那份活锁
  会在「改名 → 放回」这极短的窗口里对别的进程不可见；放回失败（锁位已被第三方占住）时残骸
  **保留不删**当现场。窗口是两个相邻系统调用，且需要三方时序同时凑齐。
  不接管活锁是**有意取舍**：持有者进程挂死时，其它进程等满等待上限后如实报占用，而不是去抢——
  抢一把还活着的锁会让两个进程同时写私有仓。
  **等待上限分两档**（这是「不阻断 turn」的具体落实）：
  - **屏障上**（`agent/pre-step` 的 before 快照与容量治理）只等 `LOCK_BARRIER_TIMEOUT_MS`（20 秒）：
    这个等待挂在**用户可见的 turn 开场屏障**上（apply 要先 await 它才继续跑本轮），等不到就照常
    记一条「快照不可用」并继续——这一轮没有卡片，但用户的对话不会被邻居进程拖住；
  - **后台结算、实时读数与撤销**等 `LOCK_WAIT_TIMEOUT_MS`（5 分钟，与 git 重活同预算）：它们不在
    屏障上，等得起，草率放弃反而会丢掉卡片；撤销等不到则回 `409 TURNREWIND_WORKSPACE_BUSY`
    （可重试，卡片给文案）。
...
- **跨进程锁覆盖的是「私有仓 + 撤销」，不覆盖账本文件本身**：`sessions/<id>.json` 的
  读改写仍是**每进程**内存串行 + 原子写。两个进程同时写**同一个会话**的账本理论上仍可能丢
  一次更新（真实场景里一个会话归一个进程所有，重启交叠时才会撞上）。锁这一层先把
  代价更高的私有仓损坏堵住；账本的跨进程串行化是后续可做项。
- **不接管活锁 ⇒ 持有者挂死时该工作区会「忙」一段时间**：撤销等满 5 分钟后如实回
  `TURNREWIND_WORKSPACE_BUSY`（可重试）；**屏障上的 before 快照只等 20 秒**就放弃并照常继续
  （记一条「快照不可用」、不阻断 turn——宁可用户这一轮没卡片，也不让对话开场被拖住）；
  后台结算与实时读数等满 5 分钟。这是有意的取舍——宁可暂时不可用，也不去抢一把还活着的锁
  （抢了会让两个进程同时写私有仓）。
- **手删 `locks/` 目录（或杀毒软件改写锁文件）会让互斥在那一刻失效**：文件锁的固有限制，
  不是实现漏洞。锁目录被删后插件会重建（下一次获取自愈），但「正在跑的持有者」与「新来者」
  之间那一瞬的互斥已经无法保证。
...
- 未做：redo、父对话递归撤销、保留策略设置面、恢复围栏、撤销后给模型的一次性提示注入。
  ...
  工作区漂移绑定、中断撤销日志等）记录在
  [`docs/plugins/expired/11.优化计划.turnrewind实现.done.md`](../../docs/plugins/expired/11.优化计划.turnrewind实现.done.md) §13.3。
...
POST /api/turnrewind/undo   { sessionId, turn }
  ...
  → 409 { error: "TURNREWIND_CONFLICT" | "TURNREWIND_EXPIRED" | "TURNREWIND_TURN_ACTIVE"
                ...
                | "TURNREWIND_UNSAFE_PATH" | "TURNREWIND_NON_EMPTY_DIR"
                | "TURNREWIND_WORKSPACE_BUSY", conflicts: [...] }
  → 403 / 404 / 400
```

`TURNREWIND_WORKSPACE_BUSY` 是**可重试**的：同一个 `$DSH_HOME` 下的另一个宿主进程正占着这个
工作区（它在那里捕获或撤销），等满 5 分钟仍没轮到才报。客户端据此给出「稍后重试」文案，
而不是把它当成终态不可用。
...
**屏障上的两次加锁用短上限、后台结算仍用默认长上限**）、
被 `.gitignore` 忽略的排除路径不会让捕获整体失败、捕获限额（超限排除重试、嵌套仓库自动跳过与报错兜底）、
容量治理（prune 只清不可达对象、超限整仓重建 + 代数轮换、排除清单复检、不碰用户仓库）、
工作区队列（FIFO 不重叠 / 跨工作区不阻塞 / 队尾出队 / **每个任务都包在跨进程锁内且进锁的是归一后的键**
/ **本次的跨进程等待上限按次透传给锁** / **锁获取失败原样透出、不阻断后续排队者**）、
**跨进程工作区锁**（同工作区后到者必须等释放、不同工作区互不阻塞、等不到锁如实报超时并带线协议原因码、
**等待上限可按次覆盖**、死 pid 与空文件/半截 JSON 残骸立即接管、**活着的持有者即使锁很旧也绝不抢**、
**锁位已被换成活锁时接管连碰都不碰它（且不留残骸）**、并发抢陈旧锁（含**空残骸窗口**：交替种死 pid 与空文件）
时任意时刻只有一个持有者、
释放只删自己那一把、任务抛错也释放、**锁目录被外部删掉后自愈**、
锁位落在 `$DSH_HOME/<plugin>/locks` 且按工作区哈希一一对应）、
