...
- **打开目录**：分组标题带下划线并可点击，宿主按组内会话 id 解析
  `$DSH_HOME/sessions/...` 下对应的会话数据目录（同一工作区的会话共享项目目录，
  直接打开；跨项目回退 sessions 根目录）并在系统文件管理器中打开，失败弹 toast。
...
| POST | `/open-path` | 按组内 `sessionIds` 解析会话数据目录（`$DSH_HOME/sessions/...`）并在系统文件管理器中打开 |
| POST | `/archive` | 归档单个会话 |
