...
- **后端**：Rust / Tauri 2 (`src-tauri/src/`)
  ...
  - `service/download/`：Node.js / Dsh 下载解压
  - `service/workflow/`：进程生命周期管理（Windows 无窗口启动：`win_spawn.rs`）
  - `service/cli/`：命令行集成（生成 `dsh` shim 脚本并写入 PATH）
...
* 脚本位置：Win `%LOCALAPPDATA%\deepseek-harness\bin`，Unix `~/.local/bin/dsh`。
...
* **dsh 结构**：`dsh` CLI 为 Node 脚本（`dependencies/dsh/node_modules/@deepseek-ai/dsh/lib/bin.js`），命令行集成本质是 **shim + PATH**。
* **AppData 结构**：`runtime/node.exe`、`dependencies/dsh/`、`data/dsh` (DSH_HOME)、`logs/dsh-web.log`。
