...
const commit = '5e25795c5d22ff9005737527e1da1ee1f8229db7';
const tag = 'dsh-0.1.5-rc.3-wanglab071';
...
try {
  ...
  if (manifest.version !== version || setting.dsh_pkg_commit !== commit || !fs.statSync(entry).isFile()) {
    throw new Error('The installed Core cannot be confirmed. Install Desktop 0.7.1 to reinstall its paired Core. No files changed.');
  }
  if (setting.dsh_pkg_tag === tag) {
    ...
    console.log('Reopen Wanglab Harness Desktop 0.7.1 or later.');
  }
} catch (error) {
