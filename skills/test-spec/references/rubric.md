...
## R15 文档与编号一致性 · 基线 major

- 用例文档条目与 `it()` **严格 1:1**；文档标题与 `it()` 标题逐字一致。
- 编号：`TC-<域>-L<层>-<文件序号>-<用例序号>`；桌面端 `TC-DSK-L3-<文件序号>-<序号>`；文件序号取 `docs/testing/<层>/<序号>-*.md`，用例序号在文件内从 `001` 起且**不跨文件连续**。
- 行为改动必须同步 `docs/testing/**` 文档 + `docs/testing/progressive.md` §4 台账 + `docs/testing/history.md`。
- 只加代码不加文档 = major；编号跳号/跨文件续号 = major。
...
| **major** | 会漏掉真实缺陷或在重构时误红：弱断言、实现耦合、过度 mock、隔离缺失、缺关键边界、文档不同步 | 本 PR 内修，或明确记 issue 并说明风险 |
