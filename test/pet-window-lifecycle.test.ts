...
describe('pet window close (destroy, not hide)', () => {
  ...
  it('never creates a hidden pet window while the pet is disabled', () => {
    ...
    expect(body).toContain('restore_pet')
...
describe('closing the pet persists (restart must not re-open it)', () => {
  ...
  it('persists enabled=false and stops the host session stream together', () => {
    ...
    // 新窗口完成媒体解码并挂好监听后才订阅，避免错过初始会话快照。
    expect(functionBody(bridge, 'report_pet_render')).toContain('sync_pet_session_stream(&app, status.enabled && status.visible && status.ready)')
    const restore = functionBody(bridge, 'restore_pet')
    expect(restore).toContain('if setting.pet_enabled')
    expect(restore).toContain('migrate_pet_selection')
  })
