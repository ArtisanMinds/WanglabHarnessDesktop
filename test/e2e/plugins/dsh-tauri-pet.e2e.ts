...
describe('L2 客户端', () => {
  ...
  it('验证 iframe 内桌宠设置分区正常渲染且无崩溃', async () => {
    ...
    try {
      ...
      expect(panelState.tabs.map(tab => tab.text), '标签必须仅包含宠物与市场').toEqual(['宠物', '市场'])
      ...
      expect(panelState.sizeSlider!.min, '尺寸下限必须与插件常量一致').toBe('25')
  ...
  it('验证设置菜单里克隆出唯一的桌宠条目且状态可读', async () => {
    ...
    try {
      ...
      // 本层（浏览器态 iframe，无 Tauri 宿主）`invoke` 必然失败，插件读不到已选宠物，
      // 因而必须给出明确引导并禁用开关，不能向宿主提交一个必然失败的启用请求。
      const label = await readMenuLabel(item)
      expect(label, '未选择宠物时必须引导用户先进入宠物设置').toMatch(/^(请先在宠物设置中选择一只宠物|Choose a pet in pet settings first)$/)
      expect(await item.isDisabled(), '未选择宠物时菜单开关必须禁用').toBe(true)
      expect(
    ...
    try {
      ...
      for (let round = 1; round <= 3; round += 1) {
        ...
        await expect.poll(
          ...
          { timeout: 15_000, message: `第 ${round} 轮 Escape 后菜单必须收起，否则重开路径未被验证` },
        ).toBe('false')
