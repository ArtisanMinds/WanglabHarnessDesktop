...
export function ConfigPlugin() {
  ...
  const displayPlugins = [...plugins.filter(plugin => !plugin.internal), ...internalPlugins]
          ...
          <div className="flex flex-col gap-4">
            {displayPlugins.map(plugin => (
              <Fragment key={plugin.id}>
                ...
                <If cond={plugin === internalPlugins[0]}>
                  <button
                    type="button"
                ...
                <Item
                  key={plugin.id}
                  className={plugin.internal && !showInternal ? 'hidden' : undefined}
                  left={(
