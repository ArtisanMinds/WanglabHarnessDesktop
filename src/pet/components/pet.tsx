...
import type { PetAnimationTarget } from '../pet-config'
...
export function Pet(props: PetProps) {
  ...
  const appliedRef = useRef<{ renderId: number, target: PetAnimationTarget } | null>(null)
  ...
  useEffect(() => {
    ...
    // 新一轮唤醒必须重新报告媒体就绪，同一轮内才对重复动画目标去重。
    if (appliedRef.current?.renderId === renderId && !shouldReloadAnimation(appliedRef.current.target, nextTarget))
      return undefined
    ...
    const onReady = () => {
      ...
      appliedRef.current = { renderId, target: nextTarget }
