...
import { useEventListener, useTimeoutFn, useWakeLock, useWatch } from '@reause/core'
...
export function App() {
  ...
  const bubble = useBubble(visible)
      ...
      <main className={`pointer-events-none fixed inset-0 flex items-end justify-center ${visible ? '' : 'invisible'}`}>
        {source && (
          <Pet
            ...
            onError={() => reportPetRender({ active_pet: activePet, render_id: renderId }, 'PET_MEDIA_DECODE_FAILED: failed to load pet media')}
          />
        )}
