 ...
 */
export function useBubble(visible: boolean): BubbleHandle {
  ...
  useListen('session:create', event => visible && tracker.apply(event.payload, 'create'))
  useListen('session:update', event => visible && tracker.apply(event.payload, 'update'))
  useListen('session:remove', event => visible && tracker.apply(event.payload, 'remove'))
